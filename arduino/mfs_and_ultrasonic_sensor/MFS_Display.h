...
class MFS_Display
{
  ...
  MFS_Display(byte data, byte clk, byte latch);
    ...
    */
    ...
    /*!
      @brief Parse number and fill digit store
      @param uint8_t number - number (10 - 99) to show
     */
    byte showNice(uint8_t);
    /*!
      @brief Parse number and fill digit store, 
      last digit for suffix symbol
      @param uint8_t number - number (10 - 99) to show
     */
    byte showIntWithSuffix(int8_t number, uint8_t suffix);
    byte showUIntWithSuffix(uint8_t number, uint8_t suffix);
    /**
